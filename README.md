...
- 插件目录结构、配置文件和 KV 存储接口写法已按官方文档整理

## 当前未在本机验证项

当前环境未安装 AstrBot，因此以下部分尚未在真实框架中联调：

- 插件热加载
- 管理员权限判断是否与目标平台配置一致
- `self.context.send_message(...)` 在具体平台上的主动推送效果
- `on_astrbot_loaded()` 生命周期在目标版本中的实际触发行为

这些部分需要在真实 AstrBot 运行环境中验证。
